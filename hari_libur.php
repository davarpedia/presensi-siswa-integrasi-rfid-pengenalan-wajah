...
if (isset($_GET['action'])) {
    ...
    if ($action === 'get_events') {
        $q = "SELECT id, tanggal_mulai AS start, DATE_ADD(tanggal_selesai, INTERVAL 1 DAY) AS end, keterangan AS title FROM hari_libur";
        $result = $conn->query($q);
        $events = [];
        if ($result) {
...
$(document).ready(function() {  
  ...
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    // Ketika klik tanggal di kalender, lakukan autofill tanggal
    dateClick: function(info) {
      ...
      // Cek apakah sudah ada event di tanggal tersebut
      $.get('<?= basename(__FILE__); ?>', { action: 'get_by_date', tanggal: info.dateStr }, function(response) {
    ...
    // Saat klik pada event, input akan diisi data yang ada
    eventClick: function(info) {
      $('#formMessage').html('');
      $('#holidayStart').val(info.event.startStr);
      var endDate = new Date(info.event.end);
      endDate.setDate(endDate.getDate() - 1);
      var dd = endDate.toISOString().slice(0,10);
      $('#holidayEnd').val(dd);
      var desc = info.event.extendedProps.keterangan || info.event.title;
  ...
  // Simpan data dengan AJAX (INSERT/UPDATE)
